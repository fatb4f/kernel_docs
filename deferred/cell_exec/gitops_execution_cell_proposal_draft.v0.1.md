...
Operationally, the model holds because the cell has four essential properties:
...
## 11. Supporting Substrate and Reconcile Stack

Taken together, the supporting pieces form a complete reconcile stack, not just a set of adjacent tools.

### 11.1 Desired state and authority

The registry split gives the cell a usable **authority chain**:

- **JSON Schema** → local validity
- **CUE** → global legality and admissibility
- **Jsonnet** → deterministic projection and render
- **Python** → runtime consumption

That is the right structure for a kernel-spec-derived manifest. The manifest is not authored directly by the runtime; it is **compiled from admitted state**. The **normalized IR** is the critical bridge between legality and rendered artifacts.

### 11.2 Reconcile pipeline

The adapter-oriented view is already very close to an execution-cell pipeline:
...
That maps directly onto the reconcile phases:
...
This is reconciliation, but not of a long-lived steady-state service. It is reconciliation of a **declared transition**: a bounded iteration that must reach an admissible, reviewable end state.

### 11.3 Control and observation surfaces

The JSON extensions split gives the cell its operational surfaces:

- **JSON-RPC** → control plane
- **NDJSON** → event and evidence stream
- **JSON:API** → durable run, result, and review resources
- **JSON-LD** → semantic and provenance layer

This fits the iteration model cleanly:

- the controller issues commands,
- the run emits streamed evidence,
- run/result/review objects are persisted as resources,
- optional semantic metadata links artifacts back to authority and derivation.

This is what makes the cell operational rather than merely declarative.

### 11.4 Output conformance and review

The structured-output stack belongs in the **runtime and verification lane**, not the authority lane:

- **Pydantic / typed contracts**
- **Instructor / validated extraction**
- **Outlines or similar constrained decoding**
- **BFCL and related evaluation surfaces**

These support the point where outputs must do more than parse as valid JSON. They must be **contract-conformant**, reviewable, and measurable against evaluation criteria.

### 11.5 Clean synthesis

A GitOps execution cell can be modeled as:
...
Git authority
  ...
  -> JSON-RPC execution
  -> NDJSON evidence stream
  -> JSON:API run/review resources
  -> verify outputs against typed/runtime constraints
  -> promote or deny
```

## 12. Suggested Intermediate Representation
...
This framing unifies the stack into one architecture:
...
- **promotion is gated by evidence rather than assumed from runtime success.**

More importantly, the supporting pieces give the cell the four properties it actually needs:
...
That is why the model holds.

The cell is not just an iteration folder, runtime wrapper, or local tool bundle. It is a **reconcilable unit** with:

- declared intent,
- admitted state,
- bounded execution,
- streamed and durable evidence,
- review bundle completion,
- review-gated promotion.

The most accurate reading is that the GitOps execution cell is a **transactional reconcile unit over desired artifact state**.
